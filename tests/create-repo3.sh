...
commit "1-1" "initial"
git tag v0.1.0 -m"One commit case"
...
git tag v0.2.0 -m"Several commits case"
...
git tag v0.2.1 -m"Several commits as list case"

commit "3-1" "feat: feature1\nfix: fix1\nrefactor:refactor1\ndocs: docs1\nperf: perf1\nchore: chore1\nci: ci1\nbuild: build1\nstyle: style1\ntest: test1\nskip: skip1\nwip: wip1\nminor: minor1\ncustom: custom1"
git tag v0.3.0 -m"All types case"
...
git tag v0.4.0 -m"Several commits with merge commit"
...
git tag v0.5.0 -m"Like squash with merge commit"
